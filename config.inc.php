...
/*** search agent with a certain email ***/
define('Search_Agent_by_mail',
     ...
     where         ui.id = u.id and ui.prof_id = p.id and upper(ui.mail_institutionnel)= upper(:mail)'); ///attention to upper and small case comparation
...
define('Search_Agent_by_Name',
     ...
     where         ui.id = u.id and ui.prof_id = p.id and u.nom = (:nom) and u.prenom = (:prenom)'); 


// create utilisature


define('create_Agent3', 'declare
                     p_test number; 
                     begin
                    p_test := comptes.create_user(
                    :login,
                    :password,
                    :nom,
                    :prenom,
...
define('Search_student_By_Etablissment',
     'select  distinct u.login         "login",
             ...
             from utilisateurs u,
                    ...
                    etablissements e
               where e.CODE_RNE = (:UaiEtab)
                     and ui.id = u.id and e.id = ui.etb_id
                     and ui.prof_id = p.id  
                     and u.nom = (:Nom)
                     and u.prenom=(:Prenom)');
//-------------------------------------------------
...
//-----------------------------------------------------------
define('Search_Parent_By_Etablissment',
     'select  distinct u.login     "login",
                   comptes.formate_us7ascii(u.nom)   "nom",
                   comptes.formate_us7ascii(u.prenom) "prenom",
                   to_char(u.dt_naissance,\'RRRR-MM-DD\') "dateNaissance",
...
array(    'sitename'          =>  'SimpleSaml',
                    'url'                  =>
'https://192.168.0.223/simplesamlphp/www/*',
                    'allowedAttributes'     =>  'login,nom,prenom,dateNaissance,codePostal,categories',
                    'tokenModele'            =>  'Default',
                    'authenticationMethod'    => 'SQL',
                    'attributesProvider'    =>   SQL_FOR_PRONOTE),

array(    'sitename'          =>  'local',
                    'url'                  =>
'http://192.168.0.223/*',
                    'allowedAttributes'     =>  'login,nom,prenom,dateNaissance,codePostal,categories',
                    'tokenModele'            =>  'Default',
