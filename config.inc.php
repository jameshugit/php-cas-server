...
$CONFIG['MAIL_ADMIN']  = "pgl@erasme.org";
...
define('SQL_FOR_ATTRIBUTES', 
		'select  distinct u.login login, u.pwd pass, u.id "ENT_id", u.uid_ldap "uid",
                 case ui.prof_id
                    when 8 then enfant.etb_id
                    else ui.etb_id end "ENTPersonStructRattach",
                 case ui.prof_id
                    when 8 then null 
                    else comptes.formate_nivclasse_for_cas(c.nom) /*ui.cls_id*/ end "ENTEleveClasses",
                 ...
                 enfantuid.uid_ldap "LaclasseEnfantUid", 
	...
	where   u.login = upper(:LOGIN) and ui.id = u.id
			and ui.parent_elv_id = enfantUid.id(+) 
        	and e.id(+) = ui.etb_id
       		and ui.prof_id = p.id
        	and ui.cls_id = c.id(+)
       		and c.niv_id = n.id(+)
...
/* Cette requête ne gère que les PROFS et LES ELEVES pour les manuels scolaires numériques */
define('SQL_FOR_ATTRIBUTES_MEN', 
	...
	select  distinct u.uid_ldap "user",
                 ...
                 comptes.formate_nivclasse_for_cas( groupes_eleves.get_liste_cls_prf(ui.id) ) "Classe",
                 comptes.formate_nivclasse_for_cas( groupes_eleves.get_liste_grp_prf(ui.id) ) "Groupe"
     from   utilisateurs u, 
            utilisateurs_info ui, 
            etablissements e,  
            profil p
       ...
       u.login     "login", 
        comptes.formate_us7ascii(u.prenom) "prenom"
          from utilisateurs u,
             ...
             ');
//----------------------------------------------------------------------------
//Search parent with name, first name,  and The  son's sconet id that created the account 
// The difference with the provious request sconet id represents  the eleve accounts  attached to the parent
// where in the second sconet id represents the eleve account that created the parent 

define('Search_Parent_By_SconetID', 'select  distinct u.id,
                                      u.login     "login"
                                    from utilisateurs u,
                                    utilisateurs_info ui
                               where  ui.id = u.id and ui.Sconet_elv_id = (:elevid)
                 and upper(convert(u.nom,\'US7ASCII\')) = upper(convert(:nom,\'US7ASCII\')) and upper(convert(u.prenom,\'US7ASCII\')) = upper(convert(:prenom,\'US7ASCII\'))
                 ORDER BY u.id desc
      ');

//----------------------------------------------------------------------------
/*** search agent with a certain first and last name  ***/
define('Search_Agent_by_Name',
       'select  distinct u.login         "login",
                        comptes.formate_us7ascii(u.nom)   "nom",
                        comptes.formate_us7ascii(u.prenom) "prenom",
                        to_char(u.dt_naissance,\'RRRR-MM-DD\') "dateNaissance",
                        ...
                        utilisateurs_info ui,
                        profil p
                        where         ui.id = u.id and ui.prof_id = p.id and upper(u.nom) =upper(:nom) and upper(u.prenom) = upper(:prenom)');
//--------------------------------------------------------------------------
//*** search eleve by sconet id ***/ 
define('Search_student_By_Id',
            'select  distinct ui.id 
             from     utilisateurs_info ui 
             where ui.Sconet_elv_id=(:eleveid)');

//-------------------------------------------------------------------------
//*** create  a new user ***/
define('create_user', 'declare p_test number; 
                     begin
                      p_test := comptes.create_user(:login, :password, :nom, :prenom, null,
                                                     null, null, null, null, null, null,
                                                     2,1,:UaiEtab, null,549,null,26,1,
                                                     null,:sex,:profile, null,:mail,
                                                     null,null,null,:eleveid);
                     end;');

//--------------------------------------------------------------------------
//*** get etablissement id ***/
define('get_Etablissement_id', 'select e.id 
                                from etablissements e 
                                where e.CODE_RNE =(:UaiEtab)');

//----------------------------------------------------------------------------
//*** set ralation parent eleve ***/
define('set_relation_parent_eleve', 'declare
                       p_test number;
                     begin
...
$CONFIG['AUTHORIZED_SITES'] = array(
			...
			array(	'sitename'  			=>  'Partenaire_CNS',
					...
					'allowedAttributes' 	=>  'user, UAI, Profil, CategoDiscipline, MatiereEnseignEtab, Classe, Groupe, CodeNivFormation, NivFormation, NivFormationDiplome, Filiere, Specialite, Enseignement, Classe, Groupe',
					'tokenModel'			=>  'ent_manuels_numeriques',
					'authenticationMethod'	=> 'SQL',
					'attributesProvider'	=>	SQL_FOR_ATTRIBUTES_MEN),

			
			array(	'sitename'  			=>  'Le Livre Scolaire',
					'url'  					=>  '*://*.lelivrescolaire.fr/ent/*', // *://81.252.204.221/*'
					'allowedAttributes' 	=>  'user, UAI, Profil, CategoDiscipline, MatiereEnseignEtab, Classe, Groupe, CodeNivFormation, NivFormation, NivFormationDiplome, Filiere, Specialite, Enseignement, Classe, Groupe',
			//		'tokenModel'			=>  'ent_manuels_numeriques',
					'authenticationMethod'	=> 'SQL',
					'attributesProvider'	=>	SQL_FOR_ATTRIBUTES_MEN),
      ...
      // Pronote connection.
      array(  'sitename'          =>  'Pronote',
              'url'               =>  '*://pronote.dev.laclasse.lan:*/*',
       ...
       array(  'sitename'      =>  'federation avec saml',
           'url'         =>  '*://*.laclasse.com/saml/*',
           'allowedAttributes' =>  'LOGIN,ENT_id,uid, ENTPersonStructRattach,ENTEleveClasses,ENTPersonStructRattachRNE,ENTPersonProfils,ENTEleveNivFormation'),
      ...
      array( 'sitename' => 'thematiques',
			...
			array(  'sitename'              =>  'Serveur de viescolaire',
					'url'                   =>  '*://localhost*/*',
					'allowedAttributes' 	=>  'uid,ENTPersonStructRattachRNE,ENT_id,ENTPersonProfils,ENTEleveClasses,ENTEleveNivFormation,LaclasseNom,LaclassePrenom,LaclasseDateNais,LaclasseCivilite,LaclasseSexe,LaclasseProfil,LaclasseNomClasse,LaclasseEmail,LaclasseEmailAca'),

			// Attributs pour le MAIL. /!\ le password est en clair !
			array(	'sitename'  		=>  'Mail RoundCube',
					'url'  				=>  '*://www.dev.laclasse.com/mail/*',
					'allowedAttributes' =>  'LOGIN, pass, ENT_id, LaclasseCivilite, LaclasseNom, LaclassePrenom, LaclasseEmail'),
			// dev Michel
			array(	'sitename'  		=>  'Dev Michel',
					'url'  				=>  '*://michel.erasme.lan:*/*',
					'allowedAttributes' =>  'uid,ENTPersonStructRattach,ENTEleveClasses,ENTPersonStructRattachRNE,ENTPersonProfils,ENTEleveNivFormation'),
			
			// Application de gestion des mdp /!\ Internal only...
			array(	'sitename'  		=>  'Application de gestion des mots de passe',
					'url'  				=>  'http*://admin.laclasse.lan/index.html',
					'allowedAttributes' =>  'LOGIN, LaclasseProfil')
				);
