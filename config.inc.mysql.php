...
define('MySQL_AUTH', 'select login from user u where u.login =
upper(:LOGIN) and upper(u.password) = upper(:PWD)');
...
//-------------------------------------------------------------------------
define('Mysql_Search_Parent_By_EleveId',
       'select   u.id, u.login , u.prenom , u.nom
                 from user u,
                      relation_eleve re
                where  u.id = re.user_id and re.eleve_id in (select c.id from user c  where  c.id_sconet = (:elevid))
...
select   distinct u.id "user",
                 ...
                 grp.Groupe "Groupe"
        from   user u 
left join (select GROUP_CONCAT(r.lib) as "Groupe", u.id 
from user u , 
     regroupement r, 
...
UNION
    ...
    select   distinct  u.id "user",
                 ...
                 c.classes "Classe",
                 grp.groupe "Groupe"
     from   user u, (select distinct u.id as id,  GROUP_CONCAT(distinct me.libelle_court) as Matiere 
